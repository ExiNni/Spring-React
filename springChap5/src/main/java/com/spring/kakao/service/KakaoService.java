...
//http HEADER에 내 정보를 흘려 보내겠다 작성
		...
		try {
			...
			/*
			 * 카카오 로그인 아이디 + 시크릿키 + 코드 + 리다이렉트 url 모두 붙여줄 것 MultiValueMap Spring에서 제공하는
			 * 인터페이스 여러 개의 값을 하나에 키에 연결할 수 있도록 합쳐주는 역할 http 에서 요청이 여러개일 때 자주 사용 <String,
			 * String> 인 이유는 key, value를 각각 String으로 넣을 것이기 때문 MultiValueMap<key,value> key와
			 * value를 String으로 설정해서 카카오톡에서 설정한 값을 적고 그 값에 대한 내용물을 갖고오겠다는 의미 예를 들어
			 * MultiValueMap<String, String> params = new LinkedMultiValueMap<>();
			 * param.add("key1","value1"); param.add("key1","value2");
			 * param.add("key2","value3"); 결과 key1 -> [value1, value2] key2 -> [value3]
			 */
...
//요청보낼 URI
					...
					KAKAO_AUTH_URI + "/oauth/token", HttpMethod.POST, httpEntity, String.class);
	...
	private KakaoDTO getUserInfoWithToken(String accessToken) throws Exception {
		...
		return KakaoDTO.builder().id(id).email(email).nickname(nickname).name(name).gender(gender).build();
	}
